...
### update the un_hk_bridge code 
un_hk_bridge[nrow(un_hk_bridge) + 1,] = c(subset(un_hk_bridge,hkhs_code==111)$un_code,498,'United States')
un_hk_bridge[nrow(un_hk_bridge) + 1,] = c(subset(un_hk_bridge,hkhs_code==111)$un_code,451,'United States')
un_hk_bridge[nrow(un_hk_bridge) + 1,] = c(subset(un_hk_bridge,hkhs_code==225)$un_code,227,'Italy')
...
## Aggregate by origin, consignee, and commodities 
library(dplyr)
### 2017 data
hkrx_2017_final = hkrx_2017 %>%
  group_by(k, origin_un,consig_un) %>%
  summarise_at(c('vrx_hkd','vrx_un'),sum)

## keep record that is complete (no 'na' value in it)
hkrx_2017_final <- na.omit(hkrx_2017_final)

## create random number in 'origin_hk' and 'consig_hk' to keep the structure
hkrx_2017_final$origin_hk <- sample(100:999, nrow(hkrx_2017_final), replace=TRUE)
hkrx_2017_final$consig_hk <- sample(100:999, nrow(hkrx_2017_final), replace=TRUE)

hkrx_2017_final$t = "2017"
col_order <- c("t", "k", "origin_hk",
               "consig_hk", "vrx_hkd","origin_un","consig_un","vrx_un")
hkrx_2017_final <- hkrx_2017_final[, col_order]

write.csv(hkrx_2017_final, file = "hkrx_2017_final.csv",row.names=FALSE)
write.csv(hkrx_2017_final, file = "hkrx_2017_final_rowname.csv",row.names=TRUE)

### 2018 data
hkrx_2018_final = hkrx_2018 %>%
  group_by(k, origin_un,consig_un) %>%
  summarise_at(c('vrx_hkd','vrx_un'),sum)

## keep record that is complete (no 'na' value in it)
hkrx_2018_final <- na.omit(hkrx_2018_final)

hkrx_2018_final$origin_hk <- sample(100:999, nrow(hkrx_2018_final), replace=TRUE)
hkrx_2018_final$consig_hk <- sample(100:999, nrow(hkrx_2018_final), replace=TRUE)

hkrx_2018_final$t = "2018"
col_order <- c("t", "k", "origin_hk",
               "consig_hk", "vrx_hkd","origin_un","consig_un","vrx_un")
hkrx_2018_final <- hkrx_2018_final[, col_order]

write.csv(hkrx_2018_final, file = "hkrx_2018_final.csv",row.names=FALSE)
write.csv(hkrx_2018_final, file = "hkrx_2018_final_rowname.csv",row.names=TRUE)